public class Solution {
    ...
    //I first identify which numbers can actually start a consecutive sequence. 
    //A number is a valid start only if num - 1 doesn't exist in the set. 
    //From each valid start, I expand forward until the sequence ends. 
    //Since every number belongs to exactly one expansion, 
    //each number is visited at most once overall, giving O(n) time.
}
